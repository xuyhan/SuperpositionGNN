...
def plot_loss_accuracy_dual(ax, folder_loss, folder_accuracy, file_pattern="*.csv", loss_label="Loss", acc_label="Accuracy"):
    ...
    ax.set_ylabel(loss_label, fontsize=18, color="black")
    ...
    ax2.set_ylabel(acc_label, fontsize=18, color="black",)
    ax2.tick_params(axis="y", labelcolor="black", labelsize=16)
    
    # Optionally, add legends manually.
    lines_1, labels_1 = ax.get_legend_handles_labels()
    lines_2, labels_2 = ax2.get_legend_handles_labels()
    ax.legend(lines_1 + lines_2, labels_1 + labels_2, fontsize=16, loc="upper left")
    ax.grid(True)
...
def plot_multiline_from_folder(ax, folder, file_pattern="*.csv", metric_name="Metric"):
    ...
    ax.set_title(f"{metric_name} vs. Epoch", fontsize=22)
...
if __name__ == "__main__":
    ...
    folder_loss = os.path.normpath(os.path.join("GraphsForReport", "Data", "Rank", "Escaped", "Loss"))
    folder_accuracy = os.path.normpath(os.path.join("GraphsForReport", "Data", "Rank", "Escaped", "Accuracy"))
    
    # For the multi-line plot: a folder with several CSV files (for example, additional metrics)
    folder_extra = os.path.normpath(os.path.join("GraphsForReport", "Data", "Rank", "Escaped", "SingularValues"))
    ...
    axes[0].set_title("Training Loss & Accuracy", fontsize=22)
    ...
    axes[1].set_title("Singular Values Evolution", fontsize=22)
